...
RED.view = (function () {
			...
			for (var i = 0; i < moving_set.length; i++) {
				...
				for (var i = 0; i < rmlinks.length; i++) {
					...
					if (link.source == node) {
						...
						rect.on("mousedown", (function (d, n) {
							return function (d) {
								portMouseDown(d, 1, n);
							}
						})(rect, n))
							.on("touchstart", (function (d, n) {
								return function (d) {
									portMouseDown(d, 1, n);
								}
							})(rect, n))
							.on("mouseup", (function (d, n) {
								return function (d) {
									portMouseUp(d, 1, n);
								}
							})(rect, n))
							.on("touchend", (function (d, n) {
								return function (d) {
									portMouseUp(d, 1, n);
								}
							})(rect, n))
	...
	function showTouchMenu (obj, pos) {
		...
		var options = [];
		options.push({
			name: "delete", disabled: (moving_set.length === 0), onselect: function () {
				deleteSelection();
			}
		});
		options.push({
			name: "cut", disabled: (moving_set.length === 0), onselect: function () {
				copySelection();
				deleteSelection();
			}
		});
		options.push({
			name: "copy", disabled: (moving_set.length === 0), onselect: function () {
				copySelection();
			}
		});
		options.push({
			name: "paste", disabled: (clipboard.length === 0), onselect: function () {
				importNodes(clipboard, true);
			}
		});
		options.push({
			name: "edit", disabled: (moving_set.length != 1), onselect: function () {
				RED.editor.edit(mdn);
			}
		});
		options.push({
			name: "select", onselect: function () {
				selectAll();
			}
		});
		options.push({
			name: "undo", disabled: (RED.history.depth() === 0), onselect: function () {
				RED.history.pop();
			}
		});
	...
	function redraw () {
		...
		if (mouse_mode != RED.state.JOINING) {
			...
			var node = vis.selectAll(".nodegroup").data(RED.nodes.nodes.filter(function (d) {
			...
			nodeEnter.each(function (d, i) {
				...
				if (d._def.button) {
					...
					nodeButtonGroup.append('rect')
						.attr("x", function (d) {
							return d._def.align == "right" ? 10 : 5
						})
						.attr("y", 4)
						.attr("rx", 5)
						.attr("ry", 5)
				...
				for (var n = 0; n < numInputs; n++) {
					var link = RED.nodes.links.filter(function (l) {
						return (l.target == d && l.targetPort == n);
					});
					...
					if (link && link.length > 0) {
						...
						rect.on("mousedown", (function (nn) {
							return function (d) {
								portMouseDown(d, 1, nn);
							}
						})(n))
							.on("touchstart", (function (nn) {
								return function (d) {
									portMouseDown(d, 1, nn);
								}
							})(n))
							.on("mouseup", (function (nn) {
								return function (d) {
									portMouseUp(d, 1, nn);
								}
							})(n))
							.on("touchend", (function (nn) {
								return function (d) {
									portMouseUp(d, 1, nn);
								}
							})(n))
		...
		var link = vis.selectAll(".link").data(RED.nodes.links.filter(function (d) {
			...
			return d.source.id + ":" + d.sourcePort + ":" + d.target.id + ":" + d.targetPort;
		});
	...
	RED.keyboard.add(/* e */ 69, {ctrl: true}, function () {
		showExportNodesDialog();
		d3.event.preventDefault();
	});
	RED.keyboard.add(/* i */ 73, {ctrl: true}, function () {
	...
	$('#btn-export-clipboard').click(function () {
		showExportNodesDialog();
	});
	$('#btn-export-library').click(function () {
	...
	$("#node-dialog-rename-workspace form").submit(function (e) {
		e.preventDefault();
	});
